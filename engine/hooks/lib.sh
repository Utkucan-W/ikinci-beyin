...
# Kancalar <vault>/.beyin/engine/hooks içinde yaşar; vault kökü üç üst dizindir.
BEYIN_PROJECT_DIR=$(CDPATH= cd "$BEYIN_HOOK_DIR/../../.." 2>/dev/null && pwd)

BEYIN_STATE_DIR="$BEYIN_PROJECT_DIR/.beyin/engine/scripts/.state"
...
beyin_emit() {
  ...
  beyin_mark_python_missing
  BEYIN_FALLBACK='Ikinci beyin uyarisi: python3 bulunamadi. Kurulumu dogrula: python3 --version'
