...
if [ -n "$BEYIN_HOOK_INPUT" ]; then
  if command -v python3 >/dev/null 2>&1; then
    nohup python3 "$BEYIN_PROJECT_DIR/.beyin/engine/scripts/flush.py" \
      --hook-input "$BEYIN_HOOK_INPUT" >/dev/null 2>&1 &
  else
    beyin_mark_python_missing
    rm -f "$BEYIN_HOOK_INPUT" 2>/dev/null || :
    beyin_emit SessionEnd 'Arka plan oturum özeti başlatılamadı: python3 bulunamadı. Kurulumu doğrula: python3 --version'
  fi
fi
